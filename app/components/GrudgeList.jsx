...
export default function GrudgeList(){
    ...
    return (
        <div style={{margin: '20px'}}>
            ...
            <Button onClick={addGrudge} data-test="add-grudge-button">
                Add Grudge
            </Button>
            <ul style={{color: 'white', listStyleType: 'none'}} data-test="grudge-list">
                {
                    grudges.length > 0 && (
                        grudges.map((g) => {
                           return ( 
                           <li  key={g.id}>
                               <span key={g.id} style={{ color: 'black' }}>
                                     {g.text}
                               </span>
            ...
            {
                grudges.length > 0 && <Button onClick={clearGrudges}
                data-test="clear-button">Clear</Button>
            }
        </div>
    )
}